<script setup>
import {ref,defineProps, onMounted, onBeforeMount, watch,computed} from "vue"
...
<template>
  <div class="drawer">
    ...
    <div class="addMyday"><i class="iconfont icon-taiyang"></i><span v-if="taskStore.getTaskByTid(task.tid).sort === 'myday'" :style="{ color: 'rgb(50, 142, 255)' }">已添加到"我的一天"</span></div>
    <div class="dateScope">
      <div class="alert"><i class="iconfont icon-naozhong"></i><span>提醒我</span></div>
      <div class="todate" >
        <div>
          <selectDeadLine  @getDeadLine = "getDeadLine" :positionX="positionX" :positionY="positionY" :tid="task.tid">
            </selectDeadLine>
            </div>
            <span :class="{active:useDeadLine}">{{useDeadLine ? useDeadLine+' 到期':'添加截止日期'}}
              </span>
              </div>
      <div class="repeat"><i class="iconfont icon-24gl-repeatDot"></i><span>重复</span></div>
    </div>
    <div class="delete">
